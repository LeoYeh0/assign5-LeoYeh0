	...
	switch (gameState) {
		...
		// Cabbages
		  ...
		  for(int i = 0; i < cabbageX.length; i++){
			...
			&&isHit(cabbageX[i],cabbageY[i], SOIL_SIZE, SOIL_SIZE, playerX, playerY, SOIL_SIZE, SOIL_SIZE) ) { // r1 bottom edge past r2 top
     ...
     for(int i = 0; i <clockX.length ; i++){
      ...
      clockX[i] = clockY[i] = -1000;
       addTime(int(CLOCK_BONUS_SECONDS));
        }
        }
...
void addTime(float seconds){	
  gameTimer=gameTimer+60*int(seconds);
...
int getEnemyIndexByRow(int row){
    ...
    if(soldierY[1]==row* SOIL_SIZE){
    return 1;
    }
    else if(soldierY[2]==row* SOIL_SIZE){
    return 2;
    }
    else if(soldierY[3]==row* SOIL_SIZE){
    return 3;
    }
    else if(soldierY[4]==row* SOIL_SIZE){
    return 4;
    }
    else if(soldierY[5]==row* SOIL_SIZE){
    return 5;
    }
  				// Requirement #6
    else{
...
void drawCaution(){								// Requirement #6
   ...
   if (getEnemyIndexByRow(playerRow+5)==i){
